...
- 리뷰 반영: 호출 간선의 출발점이 호출 불가 정점(패키지 변수에 담긴 클로저·함수 표 — 간선이
  var에서 나감, `pkg._`)이면 그래프 도달성으로 판정(`liveSources` — DeadRTA가 비호출 심볼을
...
- 노이즈(발견): 열거형의 `const ( _ = iota )`처럼 흔한 빈 선언이 간선 없는 `pkg._`
  루트를 만들었다. 빈 선언도 이름 있는 초기화식과 같은 "모듈 심볼 참조" 조건
